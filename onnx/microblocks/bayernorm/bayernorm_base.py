...
class BayerNormBase(MicroblockBase):
    ...
    Input:  [1,4,h,w] float32 Bayer mosaic
    Output: [1,4,h,w] normalized float in [0,1]
    ...
    def build_algo(self, stage, prev_stages=None):
        ...
        vis = [
            oh.make_tensor_value_info(input_image, TensorProto.FLOAT, [1,4,"h","w"]),
        ...
        outputs = {
            #"applier": {"name": out_name, "shape": [1,4,"h","w"], "type": oh.TensorProto.FLOAT},
        ...
        result.appendOutput(out_name, shape=['n',4,"h","w"], type=oh.TensorProto.FLOAT)
        return result
