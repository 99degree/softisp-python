...
class CCMQuadraticV1(CCMBase):
    ...
    def _reshape_and_normalize(self, stage, ccm_flat, inits, nodes, vis):
        ...
        out_ccm = f"{stage}.ccm.normalized"
    ...
    # -------------------------------
    def _apply_ccm(self, stage, in_image, ccm_kernel, inits, nodes, vis):
        """
        Apply CCM kernel [3,3,1,1] to upstream image using Conv.
        Input: [n,3,h,w], CCM kernel: [3,3,1,1].
        """
        out_applier = f"{stage}.applier"
        nodes.append(
            oh.make_node(
                "Conv",
                inputs=[in_image, ccm_kernel],
                outputs=[out_applier],
        ...
        vis.append(
            oh.make_tensor_value_info(out_applier, TensorProto.FLOAT, ["n", 3, "h", "w"])
        )
        return out_applier
    ...
    def build_algo(self, stage: str, prev_stages=None):
        ...
        out_ccm_raw = self._reshape_and_normalize(stage, ccm_flat, inits, nodes, vis)  # [3,3]

        # Reshape raw CCM to kernel [3,3,1,1]
        shape_3311 = f"{stage}.shape_3311"
        inits.append(oh.make_tensor(shape_3311, TensorProto.INT64, [4], [3, 3, 1, 1]))
        out_ccm_kernel = f"{stage}.ccm"
        nodes.append(
            oh.make_node(
                "Reshape",
                inputs=[out_ccm_raw, shape_3311],
                outputs=[out_ccm_kernel],
                name=f"{stage}.reshape_ccm_kernel",
            )
        )
        vis.append(oh.make_tensor_value_info(out_ccm_kernel, TensorProto.FLOAT, [3, 3, 1, 1]))

        # Trunk 3: apply CCM kernel
        out_applier = self._apply_ccm(stage, in_image, out_ccm_kernel, inits, nodes, vis)

        # Outputs: expose both raw matrix and kernel
        outputs = {
            "ccm_raw": {"name": out_ccm_raw, "type": TensorProto.FLOAT, "shape": [3, 3]},
            "ccm": {"name": out_ccm_kernel, "type": TensorProto.FLOAT, "shape": [3, 3, 1, 1]},
            "applier": {"name": out_applier, "type": TensorProto.FLOAT, "shape": ["n", 3, "h", "w"]},
        }

        result = BuildResult(outputs, nodes, inits, vis)
        result.appendInput(in_image, type=TensorProto.FLOAT, shape=["n", 3, "h", "w"])
        result.appendInput(in_cct, type=TensorProto.FLOAT, shape=[1])
        return result
    ...
    def build_applier(self, stage, prev_stages=None):
        ...
        upstream = prev_stages[0] if prev_stages else stage
        in_image = f"{upstream}.applier"
        in_ccm   = f"{stage}.ccm"   # external input [3,3,1,1]

        out_applier = f"{stage}.applier"
        nodes.append(oh.make_node("Conv", [in_image, in_ccm], [out_applier],
                                  name=f"{stage}.conv_ccm", strides=[1,1], pads=[0,0,0,0]))

        vis += [
            oh.make_tensor_value_info(in_image, TensorProto.FLOAT, ["n",3,"h","w"]),
            oh.make_tensor_value_info(in_ccm, TensorProto.FLOAT, [3,3,1,1]),
            oh.make_tensor_value_info(out_applier, TensorProto.FLOAT, ["n",3,"h","w"]),
        ]

        outputs = {"applier": {"name": out_applier, "type":TensorProto.FLOAT, "shape":["n",3,"h","w"]}}
        result = BuildResult(outputs, nodes, inits, vis)
        result.appendInput(in_image, type=TensorProto.FLOAT, shape=["n",3,"h","w"])
        result.appendInput(in_ccm, type=TensorProto.FLOAT, shape=[3,3,1,1])
        return result
