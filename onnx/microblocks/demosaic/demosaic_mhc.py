...
class DemosaicMHC(DemosaicBase):
    ...
    def build_applier(self, stage: str, prev_stages=None):
        ...
        # 1) Slice CFA planes (input cfa4 is NCHW)
        ...
        # 2) Upsample to full resolution (planes) using CPU-friendly Resize
        ...
        # Transpose NHWC -> NCHW if needed and cast to float
        ...
        # 5) Cast masks to float (ConvTranspose already produces float, keep for consistency)
        ...
        # 6) Green interpolation at R/B sites
        ...
        vis += [
            ...
            oh.make_tensor_value_info(cfa_onehot, TensorProto.FLOAT, ["n","ht","wt",4]),
            oh.make_tensor_value_info(cfa_onehot_nchw, TensorProto.FLOAT, ["n",4,"ht","wt"]),
            oh.make_tensor_value_info(cfa_onehot_f, TensorProto.FLOAT, ["n",4,"ht","wt"]),
        ...
        result.appendInput(cfa_onehot, type=TensorProto.FLOAT, shape=["n","ht","wt",4])  # one‑hot tile (NHWC)
        return result
