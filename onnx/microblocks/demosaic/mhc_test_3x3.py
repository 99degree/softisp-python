# demosaic_mhc_like_coarse3x3.py
...
# 1) Phase transpose and split (2x2 -> coarse masks)
...
# 5) Grouped 3x3 convs for directional candidates (horizontal and vertical)
#    Use two different kernel initializers: K_h and K_v (shape [12,1,3,3], groups=12)
K_h = np.zeros((12,1,3,3), dtype=np.float32)
K_v = np.zeros((12,1,3,3), dtype=np.float32)
# Simple directional initializers: horizontal favors left/right averaging, vertical favors up/down
# Fill kernels with small directional weights for test (not learned)
for i in range(12):
    # horizontal kernel: [[0,0,0],[0.5,0,0.5],[0,0,0]]
    K_h[i,0,1,0] = 0.5
    K_h[i,0,1,2] = 0.5
    # vertical kernel: [[0,0.5,0],[0,0,0],[0,0.5,0]] (up/down)
    K_v[i,0,0,1] = 0.5
    K_v[i,0,2,1] = 0.5
...
# 8) Compute coarse green gradients (Sobel-like) to produce directional weight
#    Sobel horizontal and vertical kernels for green plane (3x3)
...
# We need G_coarse as input; slice raw_rgb_coarse channel 1 is raw_G (already have raw_G)
# Convolve raw_G with sobel kernels (use Conv with groups=1)
...
# directional score = mag_v - mag_h (positive => vertical stronger)
nodes.append(helper.make_node("Sub", ["mag_v", "mag_h"], ["dir_score"], name="sub_dir_score"))

# reduce spatial sum per pixel not needed; apply small conv to smooth and then sigmoid
# smoothing kernel 3x3 average
...
# 11) Blend candidates: blended = up_h * weight_full + up_v * (1 - weight_full)
# Expand weight_full to 3 channels by concatenating
nodes.append(helper.make_node("Concat", ["weight_full","weight_full","weight_full"], ["weight_full_3"], name="concat_weight3", axis=1))
# inv_weight = 1 - weight_full_3
...
# 12) Compute coarse delta maps (ΔR, ΔB) guided by green: grouped conv on raw_rgb_coarse -> out_channels=6 (2 per color: delta, confidence)
#    Use groups=3 and out_channels=6 so each color group produces 2 maps
K_aux = np.ones((6,1,3,3), dtype=np.float32) * 0.0  # small initializer; can be learned
# initialize small delta kernels: center small positive for delta, small for confidence
for i in range(3):
    # delta kernel for color i at index 2*i
    K_aux[2*i,0,1,1] = 0.0  # start zero
    # confidence kernel for color i at index 2*i+1
    K_aux[2*i+1,0,1,1] = 0.5  # initial moderate confidence
init_K_aux = numpy_helper.from_array(K_aux.astype(np.float32), name="K_aux")
initializers.append(init_K_aux)

# Conv: inputs raw_rgb_coarse, kernel K_aux, groups=3 -> aux_out [N,6,Hc,Wc]
nodes.append(helper.make_node("Conv", ["raw_rgb_coarse", init_K_aux.name], ["aux_out"], name="conv_aux", pads=[1,1,1,1], strides=[1,1], group=3))

# Split aux_out into per-color delta/confidence: [delta_R, conf_R, delta_G, conf_G, delta_B, conf_B]
...
# 13) Upsample delta_R_coarse and delta_B_coarse to full resolution (linear) and apply to blended channels
...
# 14) Split blended_rgb into channels and add delta to R and B; G remains as blended G
...
# 15) Reconstruct final RGB candidate: [corr_R, blend_G, corr_B] -> candidate_rgb
nodes.append(helper.make_node("Concat", ["corr_R","blend_G","corr_B"], ["candidate_rgb"], name="concat_candidate", axis=1))

# 16) Build mask_known_full from phase masks (R=p00, G=p01+p10, B=p11) at full res
...
# final = base_nearest * mask_known_full + candidate_rgb_r * (1 - mask_known_full)
# Expand mask to same shape if needed (reshape)
...
graph = helper.make_graph(
    nodes,
    "demosaic_mhc_like_coarse3x3",
    inputs=[input_raw, input_phase],
    outputs=[output_info],
    initializer=initializers
)

# Set opset/IR to 11 for compatibility
model = helper.make_model(graph, producer_name="demosaic_mhc_like")
...
# Save model
onnx.save(model, "demosaic_mhc_like_coarse3x3.onnx")
print("Saved ONNX model: demosaic_mhc_like_coarse3x3.onnx")

# -------------------------
# Run the model with onnxruntime
# -------------------------
sess = ort.InferenceSession("demosaic_mhc_like_coarse3x3.onnx", providers=["CPUExecutionProvider"])
