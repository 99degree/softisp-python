# demosaic_mhc_like_coarse3x3_opset16.py
...
# For Split nodes in opset >= 13/16, 'split' is provided as an input tensor (not attribute).
# Create split initializers for the various splits we need.
...
# Split phase into 4 channels: inputs = [data, split]
...
# 6) Split conv outputs into per-color 4-channel blocks (use split initializer)
...
# 9) Upsample weight_coarse to full resolution
...
# 12) Compute coarse delta maps (ΔR, ΔB) guided by green: grouped conv producing 6 maps
...
# Split aux_out into six maps (use split initializer)
...
# 13) Upsample delta_R_coarse and delta_B_coarse to full resolution
...
# 14) Split blended_rgb into channels and add delta to R and B
...
# 15) Reconstruct final candidate_rgb
...
graph = helper.make_graph(
    nodes,
    "demosaic_mhc_like_coarse3x3_opset16",
...
# Set opset to 16
model = helper.make_model(graph, producer_name="demosaic_mhc_like_opset16")
...
# Save model
onnx.save(model, "demosaic_mhc_like_coarse3x3_opset16.onnx")
print("Saved ONNX model: demosaic_mhc_like_coarse3x3_opset16.onnx")

# -------------------------
# Run the model with onnxruntime
# -------------------------
sess = ort.InferenceSession("demosaic_mhc_like_coarse3x3_opset16.onnx", providers=["CPUExecutionProvider"])
