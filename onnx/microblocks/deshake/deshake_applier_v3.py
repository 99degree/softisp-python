...
class DeshakeApplierV3(MicroblockBase):
    """
    DeshakeApplierV3 (v3) - 16x16 Mesh Warp Applier
    -----------------------------------------------
    APPLIER: Apply 16x16 mesh-based warp for stabilization.
    
    Position: After grid generator, applies mesh-based warp.
    
    Purpose: Apply 16x16 mesh-based warp for real-time stabilization
             with edge filling to hide black bars.
    
    Needs:
        - current_frame [n,3,h,w] : Current video frame (YUV or RGB)
        - mesh_grid [16,16,2] : 16x16 mesh vertex grid from grid generator
    ...
    Behavior:
        ...
        - build_coordinator: Not used (grid generator handles mesh)
        - build_applier: Apply 16x16 mesh-based warp

    Mesh-Based Warp:
        Uses 16x16 vertex grid for mesh-based warp. The GPU uses these
        vertices to stretch the image like a texture on a flexible 3D mesh.
        
        Benefits:
        - Much faster than per-pixel mapping
        - Enables real-time 4K 60fps stabilization
    ...
    Use Case: 16x16 mesh-based warp applier
    """
    name = 'deshake_applier_v3'
    family = 'deshake_applier_v3'
    version = 'v3'
    ...
    def build_applier(self, stage: str, prev_stages=None):
        """
        Apply 16x16 mesh-based warp with edge filling.
        """
        vis, nodes, inits = ([], [], [])
        ...
        # Upsample 16x16 mesh grid to full resolution
        ...
        # Apply GridSample for mesh-based warp
        ...
        result.appendInput(mesh_grid, type=TensorProto.FLOAT, shape=[16, 16, 2])
