...
class DeshakeGridGenerator(MicroblockBase):
    """
    DeshakeGridGenerator (v3) - Stateless ONNX 16x16 Grid Generator
    ---------------------------------------------------------------
    COORDINATOR: Generate 16x16 mesh grid from correction matrix.
    
    Position: Between CPU coordinator and applier, generates mesh grid.
    
    Purpose: Generate 16x16 mesh vertex grid from correction matrix,
             camera intrinsics, and GDC coefficients for mesh-based warp.
    
    Needs:
        - correction_matrix [3,3] : 3x3 correction matrix from CPU coordinator
    ...
    Provides:
        - mesh_grid [mesh_h,mesh_w,2] : 16x16 mesh vertex grid for warp
        - valid_mask [mesh_h,mesh_w] : Valid region mask

    Behavior:
        - build_algo: Not used (algo extracts homography from frames)
        - build_coordinator: Generate 16x16 mesh grid
        - build_applier: Not used (applier handles application)

    Grid Generation Steps:
        1. Create 16x16 normalized identity grid
        2. Apply GDC distortion to grid points
        3. Apply correction matrix transformation
        4. Output 16x16 mesh grid for GPU warp

    Transformation Composition:
        P_final = K * R_corr * K_inv * P_source
        
        Where:
        - K: Camera intrinsic matrix
        - R_corr: Correction matrix from CPU coordinator
        - P_source: Source pixel coordinates
        - P_final: Final pixel coordinates for sampling

    Complexity: ~40-50 ONNX nodes
    Use Case: Stateless ONNX grid generator for mesh-based warp
    """
    name = 'deshake_grid_generator'
    family = 'deshake_grid_generator'
    version = 'v3'
    ...
    def build_coordinator(self, stage: str, prev_stages=None):
        """
        Generate 16x16 mesh grid from correction matrix.
        """
        vis, nodes, inits = ([], [], [])
        upstream = prev_stages[0] if prev_stages else stage
        
        correction_matrix = f'{upstream}.correction_matrix'
        ...
        # Extract correction matrix elements
        r00 = f'{stage}.r00'
        r01 = f'{stage}.r01'
        r02 = f'{stage}.r02'
        r10 = f'{stage}.r10'
        ...
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r00],
                                  name=f'{stage}.slice_r00',
                                  starts=[0, 0], ends=[1, 1], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r01],
                                  name=f'{stage}.slice_r01',
                                  starts=[0, 1], ends=[1, 2], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r02],
                                  name=f'{stage}.slice_r02',
                                  starts=[0, 2], ends=[1, 3], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r10],
                                  name=f'{stage}.slice_r10',
                                  starts=[1, 0], ends=[2, 1], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r11],
                                  name=f'{stage}.slice_r11',
                                  starts=[1, 1], ends=[2, 2], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r12],
                                  name=f'{stage}.slice_r12',
                                  starts=[1, 2], ends=[2, 3], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r20],
                                  name=f'{stage}.slice_r20',
                                  starts=[2, 0], ends=[3, 1], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r21],
                                  name=f'{stage}.slice_r21',
                                  starts=[2, 1], ends=[3, 2], axes=[0, 1]))
        nodes.append(oh.make_node('Slice', inputs=[correction_matrix], outputs=[r22],
                                  name=f'{stage}.slice_r22',
                                  starts=[2, 2], ends=[3, 3], axes=[0, 1]))
        
        # Apply correction transformation to GDC-distorted points
        # [x', y', w']^T = R_corr * [x_gdc, y_gdc, 1]^T
        ones = f'{stage}.ones'
        ...
        result.appendInput(correction_matrix, type=TensorProto.FLOAT, shape=[3, 3])
